...
/// Absoluteの場合繰り返す
fn update_randomly1<R: Rng>(schedule_prop: &ScheduleProp, schedule_state: &ScheduleState, schedule: &Schedule, rng: &mut R) -> Schedule {
    ...
    if schedule_state[rx][ry] != ShiftState::Absolute {
        new_schedule[rx][ry] = [Shift::N, Shift::K, Shift::I, Shift::A, Shift::O, Shift::H][rng.gen_range(0..6)];
        new_schedule
    } else {
...
/// ランダムな1つの枠をランダムな枠に変える
fn update_randomly2<R: Rng>(schedule_prop: &ScheduleProp, schedule_state: &ScheduleState, schedule: &Schedule, rng: &mut R) -> Schedule {
    let mut new_schedule = schedule.clone();
    let rx: usize = rng.gen_range(0..schedule_prop.staff_count);
    let ry: usize = rng.gen_range(schedule_prop.buffer..schedule_prop.day_count);
    if schedule_state[rx][ry] != ShiftState::Absolute {
...
fn update_randomly4<R: Rng>(schedule_prop: &ScheduleProp, schedule_state: &ScheduleState, schedule: &Schedule, rng: &mut R) -> Schedule {
    let mut new_schedule = schedule.clone();
    let rx: usize = rng.gen_range(0..schedule_prop.staff_count);
    let ry: usize = rng.gen_range(schedule_prop.buffer..schedule_prop.day_count);
    let b1 = schedule_state[rx][ry] != ShiftState::Absolute;
    let b2 = schedule[rx][ry] == Shift::N || schedule[rx][ry] == Shift::O || schedule[rx][ry] == Shift::H || schedule[rx][ry] == Shift::U;
    if b1 && b2 {
...
fn remove_random<R: Rng>(shift: Shift, schedule_prop: &ScheduleProp, new_schedule: &mut Schedule, r: usize, rng: &mut R) {
...
fn update_randomly5<R: Rng>(schedule_prop: &ScheduleProp, schedule_state: &ScheduleState, schedule: &Schedule, rng: &mut R) -> Schedule {
    let mut new_schedule = schedule.clone();
    for r in 0..schedule_prop.staff_count {
        ...
        // IAKの連続が崩れていないか
        let b3 = iak_renzoku(schedule_prop, schedule, r, &1000.0) >= iak_renzoku(schedule_prop, &new_schedule, r, &1000.0);
